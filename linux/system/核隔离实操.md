...
sudo perf record -e context-switches -C 5-13 -- sleep 10
sudo perf script 或者 report

syscount: 按类型和进程统计系统调用次数
/usr/share/bcc/tools/syscount -p $PID -i 3 -L
-L collect syscall latency

funclatency: 跟踪指定函数，打印耗时
/usr/share/bcc/tools/funclatency -p 69588 -i 3 -m -T '*clock_gettime*' -F
-m ms
-T timestamp
-F show a separate histogram per function

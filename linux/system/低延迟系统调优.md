...
idle=poll: 禁用Linux的默认空闲状态处理程序，改为使用轮询方式。
...
可以使用以下命令来查看当前参数，`sysctl xxxxx`, 例如:
```bash
$ sysctl net.ipv4.tcp_rmem
$ sysctl net.core.rmem_max
```

以下永久生效的生效方式是修改或者添加选项到 `/etc/sysctl.conf` ，然后执行 `sysctl -p` 。  
临时手动修改方式为 `sysctl -w 参数名=参数值`

...
使用 phc2sys 可将系统时钟同步到网卡上的 PTP 硬件时钟 (PHC)。系统时钟被视为从属时钟，而网卡上的时钟则为主时钟。
...
-d [dev]：主 PPS 设备。
-s [dev|name]：主时钟。
-O [offset]：从主时间偏移量，默认为 0。PTP 按国际原子时 (TAI) 运行，而系统时钟使用的是协调世界时 (UTC)。如果您不指定 -w 来等待 ptp4l 同步，可以使用 -O 来指定 TAI 与 UTC 之间的偏差（以秒为单位）
-w：等待 ptp4l。
通用选项：
